  ...
  try {
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are a precise code patch engine. Return only valid JSON.',
        },
    ...
    res.json({
      updatedContent: parsed.updatedContent?.trim() || content,
      summary: parsed.summary?.trim() || 'Patch proposal prepared.',
    });
  } catch (error) {
